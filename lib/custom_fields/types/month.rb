    ...
    module Month
      ...
      module Target
        ...
        module ClassMethods
          ...
          #
          def apply_month_custom_field(klass, rule)
            name = rule['name']

            klass.field name, type: ::Date, localize: rule['localized'] || false

            # other methods
            klass.send(:define_method, :"formatted_#{name}") { _get_formatted_month(name) }
            klass.send(:define_method, :"formatted_#{name}=") { |value| _set_formatted_month(name, value) }
          ...
          #
          def month_attribute_get(instance, name)
          ...
          #
          def month_attribute_set(instance, name, attributes)
        ...
        def _set_formatted_month(name, value)
          if value.is_a?(::String) && !value.blank?
            date = ::Date._strptime(value, self._formatted_month_format)
        ...
        def _get_formatted_month(name)
          self.send(name.to_sym).strftime(self._formatted_month_format) rescue nil
        end

        def _formatted_month_format
