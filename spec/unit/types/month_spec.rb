describe CustomFields::Types::Month do
  ...
  context '#localize' do
    ...
    before(:each) do
      field = @blog.posts_custom_fields.build label: 'Visible at', type: 'month', localized: true
  ...
  describe 'getter and setter' do
    ...
    it 'returns an empty hash if no value has been set' do
      ...
      expect(@post.class.month_attribute_get(@post, 'posted_at')).to eq expected
    end
    ...
    it 'sets a nil value' do
      expect(@post.class.month_attribute_set(@post, 'posted_at', {})).to be_nil
    end

    it 'sets a value' do
      @post.class.month_attribute_set(@post, 'posted_at', { 'posted_at' => '2010-06-28' })

      expect(@post.posted_at).to eq Date.parse '2010-06-28'

      @post.class.month_attribute_set(@post, 'posted_at', { 'formatted_posted_at' => '2010-06-29' })
  ...
  def build_blog
    Blog.new(name: 'My personal blog').tap do |blog|
      field = blog.posts_custom_fields.build label: 'Posted at', type: 'month'
